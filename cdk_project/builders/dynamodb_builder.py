...
def add_gsis(table: dynamodb.Table, gsis: list[dict]) -> None:
    ...
    Each GSI dict must have:
    - index_name: string
    - partition_key: dict with "name" and "type" fields
    - projection: string (ALL, KEYS_ONLY, or INCLUDE)
    
    Optional fields:
    - sort_key: dict with "name" and "type" fields
    - rcu: int (only used if table is PROVISIONED)
    - wcu: int (only used if table is PROVISIONED)
    """
    for i, g in enumerate(gsis or []):
        # Validate required fields
        ErrorHandler.validate_required_fields(g, ["index_name", "partition_key", "projection"], f"GSI #{i}")
        
        pk = g["partition_key"]
        ErrorHandler.validate_field_structure(g, "partition_key", ["name", "type"], f"GSI #{i}")
        
        sk = g.get("sort_key")
        if sk:
            ErrorHandler.validate_field_structure(g, "sort_key", ["name", "type"], f"GSI #{i}")
...
@ValidationDecorators.validate_required_config_fields(["table_name", "partition_key", "billing_mode", "pitr", "kms_alias"], context="Table")
def build_table(scope: Construct, name: str, conf: dict, *, env_name: str) -> dynamodb.Table:
    ...
    - stream: string
    - rcu: int (only used if billing_mode is PROVISIONED)
    - wcu: int (only used if billing_mode is PROVISIONED)
    - global_secondary_indexes: list
    - tags: dict
    """
    # Validate partition_key structure
    ErrorHandler.validate_field_structure(conf, "partition_key", ["name", "type"], f"Table configuration for {name}")
    
    # Validate sort_key structure if present
    sk = conf.get("sort_key")
    if sk:
        ErrorHandler.validate_field_structure(conf, "sort_key", ["name", "type"], f"Table configuration for {name}")
    ...
    table = dynamodb.Table(
        ...
        partition_key=dynamodb.Attribute(name=conf["partition_key"]["name"], type=to_attr_type(conf["partition_key"]["type"])),
        sort_key=dynamodb.Attribute(name=sk["name"], type=to_attr_type(sk["type"])) if sk else None,
