...
def add_gsis(table: dynamodb.Table, gsis: list[dict]) -> None:
    ...
    for i, g in enumerate(gsis or []):
        ...
        table.add_global_secondary_index(
            ...
            sort_key=dynamodb.Attribute(name=sk["name"], type=to_attr_type(sk["type"]))
            if sk
            else None,
...
def build_table(scope: Construct, name: str, conf: dict, *, env_name: str) -> dynamodb.Table:
    ...
    table = dynamodb.Table(
        ...
        partition_key=dynamodb.Attribute(
            name=conf["partition_key"]["name"], type=to_attr_type(conf["partition_key"]["type"])
        ),
        sort_key=(
            dynamodb.Attribute(name=sk["name"], type=to_attr_type(sk["type"]))
            if (sk := conf.get("sort_key"))
            else None
        ),
