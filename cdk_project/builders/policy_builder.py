        ...
        if policy.startswith("arn:"):
            role.add_managed_policy(
                iam.ManagedPolicy.from_managed_policy_arn(
                    role, f"Managed-{policy.split('/')[-1]}", policy
                )
            )
        else:
...
def _validate_config(raw: dict) -> None:
    ...
    if not isinstance(raw, dict):
        raise TypeError("Policy config must be a dictionary")
    ...
    if "managed" in raw:
        if not isinstance(raw["managed"], (list, tuple)):
            raise TypeError("Managed policies must be a list")

    inline = raw.get("inline", {})
    if not isinstance(inline, dict):
        raise TypeError("Inline policies must be a dictionary")
    ...
    for name, stmts in inline.items():
        ...
        if not lst:
            raise ValueError(f"Inline policy '{name}' must have at least one statement")

        for i, s in enumerate(lst):
            if not isinstance(s, dict):
                raise TypeError(f"Statement #{i} in '{name}' must be a dictionary")

            # Validate required fields for IAM statement
            if "Effect" not in s or "Action" not in s:
                raise ValueError(f"Statement #{i} in '{name}' must include Effect and Action")
